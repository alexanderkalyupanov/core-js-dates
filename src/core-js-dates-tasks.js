...
function getDayName(date) {
  ...
  const dayInd = new Date(date).getDay();
  return days[dayInd];
}
...
function getNextFriday(date) {
  ...
  let daysAdd = (5 - day + 7) % 7;
  if (daysAdd === 0) daysAdd = 7;
  const nextFriday = new Date(data);
  nextFriday.setDate(data.getDate() + daysAdd);
  return nextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (secondDate - firstDate) / 86400000 + 1;
}
...
function formatDate(date) {
  ...
  const minutes = String(data.getMinutes()).padStart(2, '0');
  const seconds = String(data.getSeconds()).padStart(2, '0');
  return `${month}/${day}/${year}, ${formattedHours}:${minutes}:${seconds} ${timeStatus}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const date = new Date(year, jsMonth + 1, 0).getDate();
  let counter = 0;
  for (let i = 1; i <= date; i += 1) {
    const currentDate = new Date(year, jsMonth, i);
